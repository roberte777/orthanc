...
//
// The base URL is runtime-configurable via `set_base_url()`. The web UI sets it
// once at startup from a build-time constant; the mobile UI captures it from a
// server-config screen and persists it to platform storage. Reads after the
// first set are lock-free.
...
/// Set the API base URL. Only the first call wins; subsequent calls are no-ops.
/// To change the URL after first set, restart the app.
pub fn set_base_url(url: String) {
    let _ = API_BASE_URL.set(url);
}

/// Get the configured API base URL. Returns a sensible localhost default if
/// `set_base_url` was never called (useful for tests and dev defaults).
...
pub async fn stop_transcode(token: &str, session_id: &str) -> Result<(), String> {
    let url = format!("{}/api/media/transcode/{}", base_url(), session_id);
...
pub async fn update_progress(
    ...
    let url = format!("{}/api/media/{}/progress", base_url(), media_id);
    let client = reqwest::Client::new();
    let resp = client
        .put(&url)
        .json(&body)
...
pub async fn save_track_preferences(
    ...
    let url = format!("{}/api/media/track-preferences", base_url());
    let client = reqwest::Client::new();
    let resp = client
        .put(&url)
        .json(&body)
