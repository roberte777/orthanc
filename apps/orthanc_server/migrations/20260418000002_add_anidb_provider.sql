-- Drop and recreate the provider CHECK to allow 'anidb'
-- SQLite doesn't support ALTER CHECK, so we recreate the table
CREATE TABLE library_metadata_providers_new (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    library_id INTEGER NOT NULL,
    provider TEXT NOT NULL CHECK(provider IN ('tmdb', 'anidb')),
...
CREATE INDEX idx_library_metadata_providers_library_v2 ON library_metadata_providers(library_id);

-- Add AniDB as disabled by default for all existing libraries
INSERT OR IGNORE INTO library_metadata_providers (library_id, provider, is_enabled, priority)
SELECT id, 'anidb', 0, 10 FROM libraries;

-- Add anidb_id column to media_items for tracking
-- (tmdb_id and tvdb_id already exist)
